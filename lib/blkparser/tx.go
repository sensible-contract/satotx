...
func NewTx(rawtx []byte) (tx *Tx, offset uint) {
	...
	txincnt, txincntsize := SafeDecodeVarIntForBlock(rawtx[offset:])
	offset += txincntsize
	...
	txoutcnt, txoutcntsize := SafeDecodeVarIntForBlock(rawtx[offset:])
	offset += txoutcntsize
...
func NewTxIn(txinraw []byte) (txin *TxIn, offset uint) {
	...
	scriptsig, scriptsigsize := SafeDecodeVarIntForBlock(txinraw[offset:])
...
func NewTxOut(txoutraw []byte) (txout *TxOut, offset uint) {
	...
	pkscript, pkscriptsize := SafeDecodeVarIntForBlock(txoutraw[offset:])
	offset += pkscriptsize
